...
function create() {
	...
	// Simple joint with anchors at the center of each sprite, and
	// using the position of the sprites to determine the joint length.
	{
        ...
        //**Since only the 2 body parameters were given, the joint defaults to body centers for anchor points and uses body positions to determine joint length
...
function update() {
	...
	if ( spriteA.body.containsPoint(game.input.mousePointer) ) {
		codeCaption.text = 'game.physics.box2d.distanceJoint(spriteA, spriteB) \nJoint length is determined by the starting position of the bodies in this case';
	} else {
        codeCaption.text = '';
    }
